...
export function parseWorkBoard(markdown: string): WorkBoard {
  ...
  while (i < lines.length) {
    ...
    if (line === '## Archived') section = 'archived';
    ...
    if (taskHeader) {
      ...
      while (i < lines.length) {
        ...
        if (metaMatch && mode === '') {
        ...
        else if (t === '#### Next Steps') mode = 'next';
        else if (t === '#### Notes') mode = 'notes';
        else if (t === '#### Worklog') mode = 'worklog';
        else if (mode === 'summary' && t && !t.startsWith('####')) summary += (summary ? '\n' : '') + cur.trim();
        else if (mode === 'notes' && t && !t.startsWith('####')) notes += (notes ? '\n' : '') + cur.trim();
          ...
          if (w) worklog.push({ date: w[1], kind: w[2] as WorklogEntry['kind'] | undefined, text: w[3] });
        }
        i++;
      }
      const task: Task = {
        ...
        created_at: meta.get('created_at') ?? new Date().toISOString(),
        updated_at: meta.get('updated_at') ?? new Date().toISOString(),
